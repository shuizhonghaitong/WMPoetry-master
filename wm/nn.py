...
def linear(inputs, output_size, bias, concat=True, dtype=None, scope=None):
    ...
    with tf.variable_scope(scope, default_name="Linear", values=[inputs]):
        if not isinstance(inputs, (list, tuple)): #isinstance，是元组中的一个返回true
            inputs = [inputs]
        ...
        output_shape = tf.concat([tf.shape(inputs[0])[:-1], [output_size]],
                                 axis=0) #注意[:-1] [batch_size,1,output_size]
        # Flatten to 2D
        ...
        if concat:
            ...
            for i in range(len(input_size)):
                ...
                results.append(tf.matmul(inputs[i], matrix)) #[batch_size,output_size]
